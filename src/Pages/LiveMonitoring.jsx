...
//   // -------------------- Start analysis --------------------
//   const startAnalysis = async () => {
//     if (!videoFile) return setErrorMessage("Please upload a video first.");
//     if (!token) return setErrorMessage("Please login first.");
...
//     try {
//       const response = await fetch(
//         "https://depi-final-project-production.up.railway.app/auth/analyze-cleaning",
//         {
//           method: "POST",
//           headers: { Authorization: `Bearer ${token}` },
//           body: formData,
...
//   const dirty_surface = reportData?.dirty_surface?.count || 0;
//   const rats = reportData?.Rats?.count || 0;
//   const insects = reportData?.Insects?.count || 0;

//   const totalViolations = dirty_surface + rats + insects;

//   const compliance = reportData
...
//         <Grid container spacing={2} sx={{ mb: 4, justifyContent: "space-around" }}>
//           {[
//             { title: "Dirty Surface", value: dirty_surface },
//             { title: "Rats", value: rats },
...
//           <Button
//             variant="contained"
//             sx={{ backgroundColor: "#b68866", "&:hover": { backgroundColor: "#700404" } }}
...
//   // ------------ Extract KPIs ------------
//   const dirty_surface = reportData?.dirty?.count || 0;
//   const rats = reportData?.rats?.count || 0;
//   const insects = reportData?.insects?.count || 0;

//   // ------------ DETECTED → Yes / No ------------
//   const ratsDetected = reportData ? (reportData.rats?.detected ? "Yes" : "No") : "No";
//   const insectsDetected = reportData ? (reportData.insects?.detected ? "Yes" : "No") : "No";
//   const dirtyDetected = reportData ? (reportData.dirty?.detected ? "Yes" : "No") : "No";
//   const cleanSurfaceDetected = reportData ? (reportData.clean_surface?.detected ? "Yes" : "No") : "No";
...
//         <Grid container spacing={2} sx={{ mb: 4, justifyContent: "space-around" }}>
//           {[
//             { title: "Dirty Surface", value: dirty_surface, detected: dirtyDetected },
//             { title: "Rats", value: rats, detected: ratsDetected },
//             { title: "Insects", value: insects, detected: insectsDetected },
//             { title: "Clean Surface", value: reportData.clean_surface?.count || 0, detected: cleanSurfaceDetected }
//           ].map((kpi) => (
//             <Grid item xs={12} sm={6} md={3} key={kpi.title}>
//               <Paper sx={{ p: 2, textAlign: "center", backgroundColor: "#191725", color: "#ffcb99" }}>
//                 <Typography sx={{ fontWeight: "bold" }}>{kpi.title}</Typography>
//                 <Typography variant="h5">{kpi.value}</Typography>
//                 <Typography sx={{ mt: 1 }}>Detected: <b>{kpi.detected}</b></Typography>
//               </Paper>
//             </Grid>
//           ))}
//         </Grid>
...
const VideoUploadAnalysis = () => {
  ...
  const startAnalysis = async () => {
    ...
    try {
      const response = await fetch(
        "https://marowael-depi.hf.space/auth/analyze-cleaning",
        {
          method: "POST",
          headers: { Authorization: `Bearer ${token}` },
          body: formData,
      ...
      const violationsFromBackend = Object.entries(data.report_data || {})
  .filter(([k]) =>
    !["total_frames_processed", "is_clean", "dirty"].includes(k)
  )
  .map(([key, value], idx) => ({
    id: idx + 1,
    type: key,
