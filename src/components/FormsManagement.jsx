...
export default function FormsManagement() {
  ...
  const handleAddForm = () => {
    ...
    const message = `
New Form Added:
Title: ${formToAdd.title}
Type: ${formToAdd.type}
Assigned To: ${formToAdd.assignedTo}
Due Date: ${formToAdd.dueDate}
    `;

    // فتح رابط WhatsApp
    const phone = formToAdd.phoneNumber.replace(/\D/g, ""); // إزالة أي رموز غير رقمية
        ...
        <Table sx={{ minWidth: 650 }}>
          ...
          <TableBody>
            {filteredForms.map((form) => (
              <TableRow
                key={form.id}
                sx={{ backgroundColor: "#f6d9d1", "&:hover": { backgroundColor: "#f5f5f5" } }}
              >
                <TableCell>{form.title}</TableCell>
                <TableCell>{form.type}</TableCell>
                <TableCell>{form.createdBy}</TableCell>
            ...
            {filteredForms.length === 0 && (
              <TableRow>
                <TableCell colSpan={9} align="center" sx={{ color: "#191725" }}>
                  No forms found.
                </TableCell>
              </TableRow>
            )}
...
>
  ...
  <Select
    ...
    label="Assign To"
  >
    <MenuItem value="Nour Soliman">Nour Soliman</MenuItem>
    <MenuItem value="Mariem">Mariem</MenuItem>
    <MenuItem value="Ziad">Ziad</MenuItem>
    <MenuItem value="Sara">Sara</MenuItem>
        ...
        <DialogActions>
          <Button onClick={handleCloseDialog} sx={{ color: "#9d0706" }}> Cancel</Button>
          <Button
